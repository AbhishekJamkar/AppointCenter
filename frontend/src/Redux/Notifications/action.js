...
import {
  ...
  FETCH_NOTIFICATIONS_BY_BUSINESS_REQUEST,
  FETCH_NOTIFICATIONS_BY_BUSINESS_SUCCESS,
  FETCH_NOTIFICATIONS_BY_BUSINESS_FAILURE,
...
// Fetch notifications by business ID
export const fetchNotificationsByBusiness = ({businessId,jwt}) => async (dispatch) => {
  dispatch({ type: FETCH_NOTIFICATIONS_BY_BUSINESS_REQUEST });
  try {
    const response = await api.get(`${API_URL}/business-owner/business/${businessId}`,
      {
        headers: { Authorization: `Bearer ${jwt}` },
        
      }
    );
    console.log("fetch business notifications", response.data)
    dispatch({ type: FETCH_NOTIFICATIONS_BY_BUSINESS_SUCCESS, payload: response.data });
  } catch (error) {
    dispatch({ type: FETCH_NOTIFICATIONS_BY_BUSINESS_FAILURE, payload: error.message });
  }
};
